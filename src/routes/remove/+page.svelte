<script lang="ts">
  ...
  async function handleProcess() {
    ...
    try {
      ...
      const result = await removePages(file, pagesToRemove);
      downloadBlob(result, `removed-${file.name}`);
    } catch {
...
<svelte:head>
  <title>PaperKit — Remove Pages</title>
</svelte:head>

<ToolLayout title="Remove Pages" description="Select pages to delete from the PDF.">
  {#if !file}
  ...
  {:else}
    ...
    <button class="btn-primary" onclick={handleProcess} disabled={processing || selectedPages.size === 0}>
      {processing ? 'Processing...' : `Remove ${selectedPages.size} Page${selectedPages.size !== 1 ? 's' : ''}`}
    </button>
  {/if}
</ToolLayout>
