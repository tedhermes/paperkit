<script lang="ts">
  ...
  import { protectPDF, downloadBlob, validateFileSize } from '$lib/pdf/process';
  ...
  async function handleProcess() {
    ...
    roadmapMessage = null;
    try {
      const result = await protectPDF(file, userPassword, ownerPassword);
      downloadBlob(result, `protected-${file.name}`);
    } catch (e) {
...
<svelte:head>
  <title>PaperKit — Protect PDF</title>
</svelte:head>

<ToolLayout title="Protect PDF" description="Add password protection. (Limited — see Roadmap)">
  {#if !file}
    <div class="empty-state">
      <span class="empty-icon">🔒</span>
      <h3>Ready to protect</h3>
      <p>Drop your PDF here to add password protection.</p>
    </div>
  ...
  {:else}
    ...
    <div class="form-group">
      ...
      <input id="upw" type="password" bind:value={userPassword} class="text-input" placeholder="Enter password" />
    </div>

    <div class="form-group">
      <label for="opw">Owner password (optional)</label>
      <input id="opw" type="password" bind:value={ownerPassword} class="text-input" placeholder="Enter owner password" />
    </div>
